...
public class TestRestrictTo extends BaseXnatRestTest {
    ...
    private class TestUrl {
        ...
        String url(Operation operation) {
            ...
            urlComponents.add("/test/restrictTo/");
            if (project != null) {
    ...
    private abstract class RestrictTest {
        ...
        void performDisallowedCall(User user, TestUrl testUrl, boolean overrideFor404) {
            ...
            queryBase(user).get(testUrl.url(operation)).then().assertThat().statusCode(expectedStatusCode).and().body(Matchers.not(Matchers.equalTo(testUrl.expectedResponse())));
        }
    ...
    private class ProjectTest extends RestrictTest {
        @Override
        public void run() {
            for (Project project : mainProjects) {
    ...
    private class SubjectTest extends ProjectTest {
    ...
    private abstract class ExperimentTest extends SubjectTest {
    ...
    private class SubjectAssessorTest extends ExperimentTest {
        @Override
    ...
    private class SessionAssessorTest extends ExperimentTest {
        @Override
        protected void populateExperimentLabelMap() {
            for (Project project : allProjects) {
                for (Subject subject : project.getSubjects()) {
